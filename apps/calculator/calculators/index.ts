...
import { clamp, round, safeCeil, safeDiv, sanitizeNum, validateWaste } from '@/utils/validate';
...
export function calcConcrete(i: ConcreteInputs): CalculationResult[] {
  ...
  const results: CalculationResult[] = [
    ...
    { label: 'Volume', value: withWaste.toFixed(2), unit: 'cu ft' },
    {
      ...
      description: i.includeWaste ? `Includes ${i.waste}% waste` : 'No waste applied',
    },
  ];
...
export function calcWireGauge(i: WireGaugeInputs): CalculationResult[] {
  ...
  const fallbackSpec = specs[specs.length - 1] ?? {
    gauge: '2/0 AWG',
    ampacity: 0,
    resistancePer1000Ft: 0,
  };

  const minimumAmpacitySpec = specs.find((spec) => spec.ampacity >= amps) ?? fallbackSpec;

  const recommendedSpec =
    specs.find((spec) => {
      if (spec.ampacity < amps) return false;

      const voltageDrop = safeDiv(roundTripDistance * amps * spec.resistancePer1000Ft, 1000);
  ...
  const voltageDrop = round(
    safeDiv(roundTripDistance * amps * recommendedSpec.resistancePer1000Ft, 1000),
    2,
  );
